...
function [m_sub, m_roi, stats] = spatial_scaling(fn, px_per_mm, result_dir, index_vals, dilate_px, delta_i)
    if nargin < 6
        delta_i = 1;
    end
    ...
    colormap_gif = [0 0 0; 0.2 0.2 0.3; parula(256-2)]; % black background, white ROI, parula colors
    colormap_analysis = [0 0 0; 1 1 1; 1 0 0]; % black background, white ROI, red veins
    
    %maxwidth = quantile(m.width(m.width>0),0.99);
    ...
    for index=1:delta_i:numel(index_vals)
        i = index_vals(index);
        fprintf(sprintf('%s %d/%d\n', fn_base, index, numel(index_vals)));

        m_this = double(m_roi>0); % set bg = 0, roi = 1
        ...
        %m_sub_this(bwareafilt(m_sub_this>0, 1)==0) = 0; % retain only the giant component
        ...
        if (dilate_px > 0)
            im_indexed = imdilate(im_indexed, strel('square',dilate_px));
        end
        ...
        fn_analysis = sprintf('%s_%08d_analysis.png', fullfile(result_dir, fn_base), i); % need %08 so the vertcat in the table code works (same # of characters)
        ...
        stats_this.index_mm = i / px_per_mm; % add spatial resolution

        % also calculate local density of veins in this size class
        count_px_roi = numel(find(m_roi == 1));
        count_px_vein = numel(find(m_sub == i));
        vein_density_local = double(count_px_vein) / double(count_px_roi) * px_per_mm^2;
        stats_this.vein_density_local = vein_density_local;
